...
void task7() {
    ...
    if (kma == (a/b) || kmb == (a/b) || kmc == (a/b)) {
        cout << "Одна или несколько медиан треугольника параллельны заданной прямой\n";
    }
    else {cout << "Ни одна из медиан треугольника параллельна заданой прямой\n";}
}
...
void task8() {
    ...
    int N, n, temp=0;                                       //предполагаю, что принимаются только четырехзначные значения
    cin >> n;
    N = n;
    for (int i=1; i >= 0; --i) {                            //int i убывает, из-за этого число в temp записывается сразу в обратном порядке
        temp += (N % 10) * pow(10, i);     
        N /= 10;
        cout << temp << endl;
    }
    if (temp == N) {cout << "Является\n";}
    else {cout << "Не является\n";}
}
